...
export async function handler(event) {
  try {
    ...
    const wf = {
      ...
      "6": {"inputs":{"text":"A dynamic, slightly angled shot of a paneer wrap...","clip":["11",0]},"class_type":"CLIPTextEncode"},
    ...
    // --- Create prediction (do NOT wait here)
    let prediction;
    try {
      ...
      // Surface Replicate's error clearly instead of 502
      return {
        statusCode: 500,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          error: apiErr?.message || "Replicate create failed",
    ...
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ error: err?.message || "Create failed", stack: err?.stack || null })
    };
  }
}
